  ...
  class ScrollPlay {
    ...
    loadSequence() {
      ...
      for (let i = 1; i <= parseInt(this.frames) - 1; i++) {
        sequence.push(this.sequence + `${`000${i}`.slice(-4)}.jpg`);
      }
      ...
      this.canvas.height = window.innerHeight;
      this.maxHeight =
        Math.floor(this.sequenceImages.length) * this.playbackConst;
      ...
      for (var i = 0; i < sequence.length; i++) {
        ...
        image.onload = function () {
          ...
          if (loadcount == 1 && preload) {
            ...
            console.log(this);
            var scale = Math.max(
              world.canvas.width / this.width,
              world.canvas.height / this.height
            );
    ...
    updatePlayHead() {
      ...
      var step = function () {
        ...
        if (world.vid) {
          ...
          var scale = Math.max(
            world.canvas.width / 1920,
            world.canvas.height / 1080
          );
          // get the top left position of the image
          ...
          var img =
            world.sequenceImages[
              clamp(world.activeImage.toFixed(0) - 1, 1, world.frames)
            ];
          if (typeof img != "undefined") {
            // get the scale
            var scale = Math.max(
              world.canvas.width / img.width,
              world.canvas.height / img.height
            );
            // get the top left position of the image
