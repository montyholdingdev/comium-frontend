<!DOCTYPE html>
<html lang="en">

<head>
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-EBLZZJYD58"></script>
  <script>
    window.dataLayer = window.dataLayer || [];

    function gtag() {
      dataLayer.push(arguments);
    }
    gtag('js', new Date());

    gtag('config', 'G-EBLZZJYD58');
  </script>
  <meta charset="utf-8" />
  <meta content="width=device-width, initial-scale=1.0" name="viewport" />
  <title>Comium</title>
  <meta content="" name="description" />
  <meta content="" name="keywords" />
  <meta name="robots" content=”noindex”>
  <!-- Favicons -->
  <link href="assets/img/logo-rd.svg" rel="icon" />

  <!-- Vendor CSS Files -->
  <link href="assets/vendor/aos/aos.css" rel="stylesheet" />
  <link href="assets/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet" />
  <!--<link rel="stylesheet" href="https://use.typekit.net/yfw8tzl.css" />-->
  <!--<link href="assets/vendor/bootstrap-icons/bootstrap-icons.css" rel="stylesheet" />-->
  <link href="assets/vendor/bootstrap-icons-new/font/bootstrap-icons.css" rel="stylesheet" />
  <link href="assets/vendor/glightbox/css/glightbox.min.css" rel="stylesheet" />
  <link href="assets/vendor/remixicon/remixicon.css" rel="stylesheet" />
  <link href="assets/vendor/swiper/swiper-bundle.min.css" rel="stylesheet" />
  <!-- Template Main CSS File -->
  <link href="assets/css/style.css?v37" rel="stylesheet" />
  <link href="assets/css/main.css?v57" rel="stylesheet" />
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600;700;800&display=swap');

    @media (min-width: 550px) and (max-width: 900px) {
      .changepic {
        height: 700px !important
      }
    }

    .service-box-new {
      height: 100%;
      padding: 60px 30px;
      text-align: center;
      transition: .3s;
      border-radius: 5px;
      box-shadow: 0 0 30px rgba(1, 41, 112, .08);
      transition: all 0.2s linear;
    }

    .service-box-new:hover {
      color: white;
    }

    .box-1:hover {
      background-color: #DA2C21;
    }

    .box-2:hover {
      background-color: #37A8E0;
    }

    .box-3:hover {
      background-color: #0C1C8C;
    }

    .box-4:hover {
      background-color: #000000;
    }

    .box-5:hover {
      background-color: #3A7728;
    }

    .box-6:hover {
      background-color: #DA2C21;
    }

    .box-7:hover {
      background-color: #0C1C8C;
    }

    .service-box-new h3 {
      font-size: 18px;
      font-weight: 700;
    }

    .icon-wrap {
      width: 75px;
      height: 75px;
      border-radius: 50%;
      background-color: white;
      text-align: center;
      display: inline-flex;
      align-items: center;
      justify-content: center;
    }
  </style>
  <link href="assets/css/slick.min.css" rel="stylesheet" />
  <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.5.9/slick-theme.min.css" />
</head>

<body style="background-color:rgb(249 249 249);">
  <!-- ======= Header ======= -->
  <header id="header" class="header fixed-top">
    <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
      <a href="/" class="logo d-flex align-items-center">
        <img src="assets/img/logo-rd.svg" alt="comium" style="height: 74px;padding: 20px 0px;" />
        <!-- <span>FlexStart</span> -->
      </a>
      <nav id="navbar" class="navbar">
        <ul>
          <li>
            <a class="nav-link hover-careers" href="https://comium.gm/#about">About <svg class="careers-svg" width="19" height="19" viewBox="0 0 19 19" stroke="black" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.125 14.25L11.875 9.5L7.125 4.75" fill="transparent" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </a>
          </li>
          <li>
            <a class="nav-link hover-careers" href="https://comium.gm/#products">Products & Services <svg class="careers-svg" width="19" height="19" viewBox="0 0 19 19" stroke="black" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.125 14.25L11.875 9.5L7.125 4.75" fill="transparent" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </a>
          </li>
          <li>
            <a class="nav-link hover-careers" href="https://comium.gm/#careers">Careers <svg class="careers-svg" width="19" height="19" viewBox="0 0 19 19" stroke="black" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.125 14.25L11.875 9.5L7.125 4.75" fill="transparent" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </a>
          </li>
          <li>
          <li>
            <a class="nav-link hover-careers" href="https://comium.gm/#contact">Contact <svg class="careers-svg" width="19" height="19" viewBox="0 0 19 19" stroke="black" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.125 14.25L11.875 9.5L7.125 4.75" fill="transparent" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </a>
          </li>
        </ul>
        <i class="bi bi-list mobile-nav-toggle"></i>
      </nav>
      <!-- .navbar -->
    </div>
  </header>
  <!-- End Header -->
  </div>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@8/swiper-bundle.min.css" />
  <script src="https://cdn.jsdelivr.net/npm/swiper@8/swiper-bundle.min.js"></script>
  <div class="main">
    <div class="wrapper" style="max-width: 100%;width:100%;">
      <div class="landing-banner" style="margin-top:calc(0vh + 75px);">
        <img class="w-100" src="./assets/img/blogarticle.webp" alt="Landing Banner">
      </div>
      <main id="main">
        <!-- ======= About Section ======= -->
        <section id="about" class="about">
          <div class="container">
            <header class="section-header pb-0 px-md-5 mx-md-5">
              <h2></h2>
              <p style="    margin: 10px 0 0 0; padding: 0;
    font-size: 38px;
    line-height: 42px;
    font-weight: 700;
    color: black;text-align:left !important;">Comium Gambia Marks Conclusion of 4G+ Testing Phase and Sets the Stage for Exceptional Connectivity Milestone</p><br><br>

              <p style="
                margin: 15px 0 30px 0;
                line-height: 30px;
                font-weight: 400;
                font-size: 17px;
                color: black;text-align:left !important;" class="">
                Comium, the disruptor in The Gambia telecom industry, is thrilled to announce the nearing completion of its 4G+ testing phase, a major step toward revolutionizing connectivity for the people of The Gambia.<br><br>
                Since its initiation, the 4G+ internet service was free of charge during testing phase. Comium has been dedicated to achieving optimal results, and the company extends its sincere gratitude to the Gambians for their valuable support and participation in this process. The commitment and collaboration of the Gambian people have played a crucial role in ensuring the success of the testing phase, and Comium is proud to have worked closely with its customers to refine and enhance the 4G+ experience.<br><br>

                As the testing phase concludes, Comium is set to reveal a groundbreaking announcement, introducing new packages that redefine affordability in 4G+ data bundles. These packages will ensure that every Gambian can enjoy tailored bundles at the most affordable rates. <br><br>

                The official announcement of this exciting new phase is scheduled to take place during a press conference at Comium's premises on January 10. The event promises to be a momentous occasion, where we will share its experience and the insights we gathered during the testing phase, how we utilized them and how we plan on taking things further in the future. <br><br>

                Moreover, Comium emphasizes that this is only the steppingstone for us, as we are working on increasing our coverage with numerous sites across all of The Gambia, especially the upcountry in the near future, as well as launching many innovative products, services, and solutions that will revolutionize the way the Gambians live their lives.<br><br>

                "We are immensely grateful to the Gambian community for their unconditional support during our 4G+ testing phase. The forthcoming announcement will showcase our commitment to providing affordable and cutting-edge connectivity solutions for the people of the Gambia," said Sarjo Khan, Comium’s Commercial Director.<br><br>

              </p>
              <!--<p style="
                margin: 15px 0 30px 0;
                line-height: 24px;
                font-weight: 700;
                font-size: 17px;
                color: #DA2C21;">Stay tuned for our updated prices on November 15, 2023.</p>-->
            </header>

          </div>
        </section>




      </main>
      <!-- End #main -->
      <!-- ======= Footer ======= -->
      <footer id="footer" class="footer">

        <div class="footer-top" style="background:#f9f9f9">
          <div class="container">
            <div class="row gy-4">
              <div class="col-lg-5 col-md-12 footer-info">
                <a href="index.html" class="logo d-flex align-items-center">
                  <img src="assets/img/logo-rd.svg" alt="footer" style="    height: 37px; " />
                </a>
                <p> At Comium work in favor of making communications ever convenient for our subscribers </p>
                <div class="social-links mt-3">
                  <a href="https://twitter.com/ComiumGm" target="_blank" class="twitter">
                    <i class="bi bi-twitter-x"></i>
                  </a>
                  <a href="https://www.facebook.com/Comium.Gambia" target="_blank" class="facebook">
                    <i class="bi bi-facebook"></i>
                  </a>
                  <a href="https://www.instagram.com/comium.gm/" target="_blank" class="instagram">
                    <i class="bi bi-instagram"></i>
                  </a>
                  <a href="https://www.linkedin.com/company/comium-gambia/" target="_blank" class="linkedin">
                    <i class="bi bi-linkedin"></i>
                  </a>
                </div>
              </div>
              <div class="col-lg-3 col-md-12 footer-contact  text-md-start useful-link">
                <h4>Useful Links</h4>
                <ul class="no-bullets">
                  <li>
                    <i class="bi bi-chevron-right"></i>
                    <a href="#products">Products & Services</a>
                  </li>
                  <li>
                    <i class="bi bi-chevron-right"></i>
                    <a href="#about">About</a>
                  </li>
                  <li>
                    <i class="bi bi-chevron-right"></i>
                    <a href="#careers">Careers</a>
                  </li>
                  <li>
                    <i class="bi bi-chevron-right"></i>
                    <a href="#contact">Contact</a>
                  </li>
                </ul>
              </div>
              <div class="col-lg-3 col-md-12 footer-contact  text-md-start">
                <h4>Contact Us</h4>
                <p> 27 Kairaba Avenue <br /> Pipeline,KSMD <br /> Banjul-The Gambia <br />
                  <strong>Phone:</strong>
                  <a href="tel:+220 6 601 111" style="color:black" onMouseOver="this.style.color='red'" onMouseOut="this.style.color='black'">+220 6 601 111</a>
                  <br />
                  <strong>Email:</strong>
                  <a href="mailto:info@comium.gm " style="color:black" onMouseOver="this.style.color='red'" onMouseOut="this.style.color='black'">info@comium.gm</a>
                </p>
              </div>
            </div>
          </div>
        </div>
        <div class="container">
          <div class="copyright"> &copy; Copyright <strong>
              <span>Comium</span>
            </strong>. All Rights Reserved </div>
          <div class="credits"></div>
        </div>
      </footer>
      <!-- End Footer -->
      <a href="#" class="back-to-top d-flex align-items-center justify-content-center">
        <i class="bi bi-arrow-up-short"></i>
      </a>


      <!-- Vendor JS Files -->
      <script src="assets/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
      <script src="assets/js/main.js?v9"></script>
      <script src="assets/js/jquery-3.5.0.min.js"></script>
      <script src="assets/js/slick.min.js"></script>

      <script src="assets/js/mixall.js?v10"></script>

      <script src="assets/vendor/aos/aos.js"></script>

      <script async src="https://www.googletagmanager.com/gtag/js?id=G-XJHHRG8NCG"></script>
      <script>
        window.dataLayer = window.dataLayer || [];

        function gtag() {
          dataLayer.push(arguments);
        }
        gtag('js', new Date());
        gtag('config', 'G-XJHHRG8NCG');
      </script>
</body>

</html>